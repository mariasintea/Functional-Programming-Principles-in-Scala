...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet = {
    that.filterAcc(elem => !this.contains(elem), this)
  }

  def mostRetweeted: Tweet = {
    def mostRetweetedTweet(elem1: Tweet, elem2: Tweet): Tweet = {
      if (elem1.retweets > elem2.retweets)
        elem1
      else
        elem2
    }
    ...
    if (left.isEmpty && right.isEmpty)
      ...
      if (left.isEmpty)
        mostRetweetedTweet(right.mostRetweeted, elem)
      else
        if (right.isEmpty)
          mostRetweetedTweet(left.mostRetweeted, elem)
        else
          mostRetweetedTweet(left.mostRetweeted, mostRetweetedTweet(right.mostRetweeted, elem))
  }

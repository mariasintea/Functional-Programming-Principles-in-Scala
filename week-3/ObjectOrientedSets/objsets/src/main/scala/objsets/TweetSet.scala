...
abstract class TweetSet extends TweetSetInterface:
  /**
   * This method checks if the TweetSet is empty
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  // use filterAcc to filter the elements that are not in that and add them to a result initialized with all elements in that
    ...
    if (left.isEmpty && right.isEmpty)
      ...
      if (left.isEmpty)
        ...
        if (right.isEmpty)
          ...
          max(left.mostRetweeted, max(right.mostRetweeted, elem))
  }
